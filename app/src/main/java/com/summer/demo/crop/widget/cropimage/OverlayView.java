 ...
 * <p/>
 * This view is used for drawing the overlay on top of the image. It may have
 * frame, crop guidelines and dimmed area. This must have LAYER_TYPE_SOFTWARE to
...
public class OverlayView extends View {
	...
	public static final int CORNER_ANGLE = 0;
	public static final int CORNER_CIRCLE = 1;
	...
	private int mCornerShape = CORNER_ANGLE;
...

	{
		...
		mCropRectCornerTouchAreaLineLength = getResources()
	...
	public void setOverlayViewChangeListener(
			OverlayViewChangeListener callback) {
		mCallback = callback;
	}
	 ...
	 *            - aspect ratio for image crop (e.g. 1.77(7) for 16:9)
	 */
	...
	/**
	 * This method setups crop bounds rectangles for given aspect ratio and view
	 * size. {@link #mCropViewRect} is used to draw crop bounds - uses padding.
	 */
	...
	private void updateGridPoints() {
		mCropGridCorners = CropRectUtils.getCornersFromRect(mCropViewRect);
		mCropGridCenter = CropRectUtils.getCenterFromRect(mCropViewRect);

		mGridPoints = null;
		mCircularPath.reset();
		mCircularPath.addCircle(mCropViewRect.centerX(),
	...
	/**
	 * Along with image there are dimmed layer, crop bounds and crop guidelines
	 * that must be drawn.
	 */
	@Override
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if ((event.getAction()
				& MotionEvent.ACTION_MASK) == MotionEvent.ACTION_MOVE) {
			if (event.getPointerCount() == 1
					&& mCurrentTouchCornerIndex != -1) {

				x = Math.min(Math.max(x, getPaddingLeft()),
						getWidth() - getPaddingRight());
				y = Math.min(Math.max(y, getPaddingTop()),
	...
	private void updateCropViewRect(float touchX, float touchY) {
		...
		// resize rectangle
		case 0:
			mTempRect.set(touchX, touchY, mCropViewRect.right,
					mCropViewRect.bottom);
			break;
		case 1:
			mTempRect.set(mCropViewRect.left, touchY, touchX,
					mCropViewRect.bottom);
			break;
		case 2:
			mTempRect.set(mCropViewRect.left, mCropViewRect.top, touchX,
					touchY);
			break;
		case 3:
			mTempRect.set(touchX, mCropViewRect.top, mCropViewRect.right,
					touchY);
			break;
		// move rectangle
		case 4:
			mTempRect.offset(touchX - mPreviousTouchX,
					touchY - mPreviousTouchY);
	...
	private int getCurrentTouchIndex(float touchX, float touchY) {
		...
		for (int i = 0; i < 8; i += 2) {
			double distanceToCorner = Math
					.sqrt(Math.pow(touchX - mCropGridCorners[i], 2)
	...
	/**
	 * This method draws crop bounds (empty rectangle) and crop guidelines
	 * (vertical and horizontal lines inside the crop bounds) if needed.
	 *
	 * @param canvas
	 *            - valid canvas object
	 */
	protected void drawCropGrid(@NonNull Canvas canvas) {
		if (mShowCropGrid) {
			if (mGridPoints == null && !mCropViewRect.isEmpty()) {
				...
				mGridPoints = new float[(mCropGridRowCount) * 4
		...
		if (mFreestyleCropMode != FREESTYLE_CROP_MODE_DISABLE) {
			...
			if (mCornerShape == CORNER_CIRCLE) {
				if (mCropViewRect.right - mCropViewRect.left != 0) {
					canvas.drawCircle(mCropViewRect.left, mCropViewRect.top,
							mCropRectCornerTouchAreaLineLength,
							mCropFrameCornersPaint);
					canvas.drawCircle(mCropViewRect.right, mCropViewRect.top,
							mCropRectCornerTouchAreaLineLength,
							mCropFrameCornersPaint);
					canvas.drawCircle(mCropViewRect.left, mCropViewRect.bottom,
							mCropRectCornerTouchAreaLineLength,
							mCropFrameCornersPaint);
					canvas.drawCircle(mCropViewRect.right, mCropViewRect.bottom,
							mCropRectCornerTouchAreaLineLength,
							mCropFrameCornersPaint);
	...
	/**
	 * This method extracts all needed values from the styled attributes. Those
	 * are used to configure the view.
	 */
	@SuppressWarnings("deprecation")
	protected void processStyledAttributes(@NonNull TypedArray a) {
		mDimmedColor = a.getColor(
				R.styleable.OverlayView_ucrop_dimmed_color,
		...
		mShowCropFrame = a.getBoolean(
				R.styleable.OverlayView_ucrop_show_frame,
				DEFAULT_SHOW_CROP_FRAME);

		initCropGridStyle(a);
		mShowCropGrid = a.getBoolean(
				R.styleable.OverlayView_ucrop_show_grid,
	...
	private void initCropFrameStyle(@NonNull TypedArray a) {
		int cropFrameStrokeSize = a.getDimensionPixelSize(
				R.styleable.OverlayView_ucrop_frame_stroke_size,
				getResources().getDimensionPixelSize(
						R.dimen.ucrop_default_crop_frame_stoke_width));
		int cropFrameColor = a.getColor(
				R.styleable.OverlayView_ucrop_frame_color, getResources()
						.getColor(R.color.ucrop_color_default_crop_frame));
	...
	private void initCropGridStyle(@NonNull TypedArray a) {
		int cropGridStrokeSize = a.getDimensionPixelSize(
				R.styleable.OverlayView_ucrop_grid_stroke_size,
				getResources().getDimensionPixelSize(
						R.dimen.ucrop_default_crop_grid_stoke_width));
		int cropGridColor = a.getColor(
				R.styleable.OverlayView_ucrop_grid_color,
		...
		mCropGridRowCount = a.getInt(
				R.styleable.OverlayView_ucrop_grid_row_count,
				DEFAULT_CROP_GRID_ROW_COUNT);
		mCropGridColumnCount = a.getInt(
				R.styleable.OverlayView_ucrop_grid_column_count,
