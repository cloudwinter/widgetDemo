...
import com.summer.demo.crop.callback.BitmapLoadCallback;
import com.summer.demo.crop.model.ExifInfo;
import com.summer.demo.crop.task.BitmapLoadTask;
 ...
 * <p/>
 * This class provides base logic to setup the image, transform it with matrix
 * (move, scale, rotate), and methods to get current matrix state.
 */
public class TransformImageView extends ImageView {
	...
	public void setTransformImageListener(
	...
	/**
	 * Setter for {@link #mMaxBitmapSize} value. Be sure to call it before
	 * {@link #setImageURI(Uri)} or other image setters.
	 *
	 * @param maxBitmapSize
	 *            - max size for both width and height of bitmap that will be
	 *            used in the view.
	 */
	...
	public int getMaxBitmapSize() {
		if (mMaxBitmapSize <= 0) {
			mMaxBitmapSize = CropBitmapUtils
					.calculateMaxBitmapSize(getContext());
		}
	...
	/**
	 * This method takes an Uri as a parameter, then calls method to decode it
	 * into Bitmap with specified size.
	 *
	 * @param imageUri
	 *            - image Uri
	 * @throws Exception
	 *             - can throw exception if having problems with decoding Uri or
	 *             OOM.
	 */
	public void setImageUri(@NonNull Uri imageUri) throws Exception {
		int maxBitmapSize = getMaxBitmapSize();

		new BitmapLoadTask(getContext(), imageUri, maxBitmapSize, maxBitmapSize, new BitmapLoadCallback() {
			...
			public void onFailure(
					@NonNull Exception bitmapWorkerException) {
				Log.e(TAG, "onFailure: setImageUri",
						bitmapWorkerException);
				if (mTransformImageListener != null) {
					mTransformImageListener
							.onLoadFailure(bitmapWorkerException);
				}
			}
		}).execute();
	}
	...
	/**
	 * @return - current image scale value. [1.0f - for original image, 2.0f -
	 *         for 200% scaled image, etc.]
	 */
	...
	public float getMatrixScale(@NonNull Matrix matrix) {
		return (float) Math
				.sqrt(Math.pow(getMatrixValue(matrix, Matrix.MSCALE_X), 2)
	...
	public Bitmap getViewBitmap() {
		if (getDrawable() == null
				|| !(getDrawable() instanceof FastBitmapDrawable)) {
			return null;
		} else {
	...
	public void postScale(float deltaScale, float px, float py) {
		if (deltaScale != 0) {
			...
			if (mTransformImageListener != null) {
				mTransformImageListener
						.onScale(getMatrixScale(mCurrentImageMatrix));
			}
		}
	}
	...
	/**
	 * When image is laid out {@link #mInitialImageCenter} and
	 * {@link #mInitialImageCenter} must be set.
	 */
	protected void onImageLaidOut() {
		...
		mInitialImageCorners = CropRectUtils.getCornersFromRect(initialImageRect);
		mInitialImageCenter = CropRectUtils.getCenterFromRect(initialImageRect);
	 ...
	 *            - valid Matrix object
	 * @param valueIndex
	 *            - index of needed value. See {@link Matrix#MSCALE_X} and
	 *            others.
	...
	/**
	 * This method logs given matrix X, Y, scale, and angle values. Can be used
	 * for debug.
	 */
	@SuppressWarnings("unused")
	protected void printMatrix(@NonNull String logPrefix,
		...
		Log.d(TAG, logPrefix + ": matrix: { x: " + x + ", y: " + y + ", scale: "
				+ rScale + ", angle: " + rAngle + " }");
	}

	/**
	 * This method updates current image corners and center points that are
	 * stored in {@link #mCurrentImageCorners} and {@link #mCurrentImageCenter}
	 * arrays. Those are used for several calculations.
	 */
	private void updateCurrentImagePoints() {
		mCurrentImageMatrix.mapPoints(mCurrentImageCorners,
				mInitialImageCorners);
