...
public class OKNetClient {
    ...
    private Request buildRequest(OKBaseRequestParam param) {
        ...
        Log.e(TAG, "buildRequest: requestParamJson:" + requestParamJson);
        ...
        if (response == null) {
            ...
            if (response.code() >= 200 && response.code() < 300) {
                try {
                    ...
                    Log.e(TAG, "callbackResult:response: " + resultStr);
                    OKResult result = parseResultJson(
                            resultStr, messageType, dataType);
    ...
    private <MessageType, DataType> OKResult<MessageType, DataType> parseResultJson(
        ...
        OKResult<MessageType, DataType> okResult = new OKResult<>();
        ...
        if (resultType != Void.class) {
            ...
            if (!TextUtils.isEmpty(content)) {
                okResult.resultData = new Gson().fromJson(content, resultType);
            }
        }
        return okResult;
    }
